...
namespace Restfull.Tests
{
    ...
    public class DeleteMessage
    {
        ...
        public void Setup()
        {
            ...
            // Nachricht 1 Löschen
            _tcp.GetData("DELETE /messages/1 HTTP/1.1\n Host: 127.0.0.1:13000\n User-Agent: insomnia/2020.4.2\nAccept: */*\nContent-Length: 0\n\n");
            _tcp.DeleteMessage(_messageQue);

            // Nachrichten Abfragen 1 bis 2
            ...
            // Falscher Index (So viele Nachrichten Existieren nicht)
            _tcp.GetData("DELETE /messages/5 HTTP/1.1\n Host: 127.0.0.1:13000\n User-Agent: insomnia/2020.4.2\nAccept: */*\nContent-Length: 0\n\n");
            _tcp.DeleteMessage(_messageQue);
            _tcp.MakeHeader();
            Result.Add(_tcp.GetHeader());
        }
        ...
        {
            Assert.AreEqual("HTTP/1.1 404 Not Found\nServer: MTCG\nContent-Length: 36\nContent-Language: de\nContent-Type: text/plain\nConnection: close\nKeep-Alive: timeout=50, max=0\nCharset: utf-8\n\nEs gibt noch nicht so viele Messages", Result[2]);
        }
    }
}